...
namespace ManagedAudioEngineUniversal.Effects
{
    public class SmbPitchShift
    {
        ...
        {
            var gInFIFO = new float[MAX_FRAME_LENGTH];
            var gOutFIFO = new float[MAX_FRAME_LENGTH];
            var gFFTworksp = new float[2*MAX_FRAME_LENGTH];
            var gLastPhase = new float[MAX_FRAME_LENGTH/2 + 1];
            var gSumPhase = new float[MAX_FRAME_LENGTH/2 + 1];
            var gOutputAccum = new float[2*MAX_FRAME_LENGTH];
            var gAnaFreq = new float[MAX_FRAME_LENGTH];
            var gAnaMagn = new float[MAX_FRAME_LENGTH];
            var gSynFreq = new float[MAX_FRAME_LENGTH];
            var gSynMagn = new float[MAX_FRAME_LENGTH];
            int gRover = 0;
        }
        ...
        /// </summary>
        public void smbPitchShift(float pitchShift, int numSampsToProcess, int fftFrameSize, int osamp, float sampleRate,
            float[] indata, float[] outdata)
        {
            ...
            expct = 2.0*M_PI_VAL*stepSize/fftFrameSize;
                ...
                i++)
            {
                ...
                if (gRover >= fftFrameSize)
                {
                    ...
                    for (k = 0;
                        k < inFifoLatency;
                        k++) gInFIFO[k] = gInFIFO[k + stepSize];
                }
            }
        }
        ...
        */
        ...
        public void smbFft(float[] fftBuffer, int fftFrameSize, int sign)
        {
            ...
            var kmax = (int) (Math.Log(fftFrameSize)/Math.Log(2.0) + 0.5);
            for (k = 0, le = 2;
                k < kmax;
                k++)
            {
